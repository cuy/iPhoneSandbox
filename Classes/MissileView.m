...
- (void)fireMissileFrom:(MountView *)mountView toEnemyMountView:(MountView *)enemyMountView {
    ...
    NSLog(@"mMissle(x,y) = (%f,%f)", mMissile.cx, mMissile.cy);
    // cx and cy are muzzle view's center
...
- (void)startFireMissile {
	...
	self.center = position;
    // rotate the missle to match the trajectory
	CGFloat rot = atan2(position.y - oposition.y, position.x - oposition.x)*180.0/M_PI;
	self.transform = CGAffineTransformMakeRotation(round(rot)*M_PI/180);
	oposition = self.center;
	
    // check if out of screen or hit the enemy mount
